...
class CTAFixedVolumeController(Strategy):

	def __init__(self):
		""" 初始化
        """
        super(CTAFixedVolumeController, self).__init__()

        # 指定这个主策略下面管理了CTAFixedVolumeModel1和CTAFixedVolumeModel2这两个模型
        self.models = [CTAFixedVolumeModel1, CTAFixedVolumeModel2]

        # CTAFixedVolumeModel1订阅‘btcusdt’， CTAFixedVolumeModel2订阅‘btcusdt’，所以需要在主策略里面同时订阅这一种
        self.symbols = ['btcusdt']

        # CTAFixedVolumeModel1关心‘btc’， CTAFixedVolumeModel2关心‘btcusdt’，所以需要在主策略里面同时订阅这一种
        self.coins = ['btc']
        ...
        # 那么状态就为running_free, 如果model想要的超过了max_money限制或者收到了max_weight限制，主策略就会按照max_money给这个model，这种情况，状态就变成了running_limited
        self.models_info = {
        					CTAFixedVolumeModel1: {
        											 'symbols': ['btcusdt'],
        											 'coins': ['btc'],
        											 'mode': 'fixed_volume',
        											 'mode_params': {
    ...
    def model_consolidation(self，can_use_asset_market_value, price_dict):
    	...
    	for model in self.models:
            ...
            money1 = can_use_asset_market_value * model.mode_params['max_weight']
            money2 = model.mode_params['max_money']
    		
    		for coin in model.coins:

    			# model需要的目标市值 = min（model固定volume * model自己的signal值 * 当前币价，model可用的最大钱数上线，总可用资产 * model最大比例）
                volume3 = model.mode_params['fixed_volume'] * model.signal[coin]
                money3 = volume3 * price_dict[coin]
                money4 = min(money1, money2, abs(money3)) * sign(money3)
                volume4 = money4 / price_dict[coin]
    			
    			# 更新self.models_info
    			self.models_info[model][status]['model_signal_want'][coin]['raw_signal'] = model.signal[coin]
    			self.models_info[model][status]['strategy_give'][coin]['market_value'] = money4
    			self.models_info[model][status]['strategy_give'][coin]['number'] = volume4
    			self.models_info[model][status]['strategy_give'][coin]['feedback_signal'] = volume4 / volume3
    			if volume3 > volume4 > 0.0 or volume3 < volume4 < 0.0:
    				self.models_info[model][status]['running_status'] = 'running_limited'

    			# 计算累计仓位市值
    			if coin not in money_dict:
    				money_dict[coin] = 0.0
    			money_dict[coin] += money4
