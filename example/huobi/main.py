...
class DemoStrategy(Strategy):
    ...
    async def on_time(self):
        ...
        if not hasattr(self, "just_once"):
            self.just_once = 1
            #xx = self.get_orders(self.gw, "eoseth")
            #xx = self.get_assets(self.gw)
            #xx = self.create_order(self.gw, "eoseth", ORDER_ACTION_SELL, "51", "0.002")
            #xx = self.create_order(self.gw, "eoseth", ORDER_ACTION_SELL, "0", "0.002", ORDER_TYPE_MARKET)
            #xx = self.revoke_order(self.gw, "eoseth", "1017521392")
            xx = self.get_symbol_info(self.gw, "eoseth")
            yy, zz = await xx
