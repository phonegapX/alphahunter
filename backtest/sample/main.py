...
class DemoStrategy(Strategy):
    ...
    def __init__(self):
        ...
        #=====================================================
        #创建第一个交易接口
        platform = config.accounts[0]["platform"] #交易所
        account = config.accounts[0]["account"] #此参数回测盘不用,不过为了统一都写上不影响
        access_key = config.accounts[0]["access_key"]  #此参数回测盘不用,不过为了统一都写上不影响
        secret_key = config.accounts[0]["secret_key"]  #此参数回测盘不用,不过为了统一都写上不影响
        symbols = config.markets[platform]["symbols"]
        # 交易模块参数
        params = {
            "strategy": config.strategy,
            #"platform": platform, #接入真实交易所
            "platform": const.BACKTEST, #接入回测平台
            "databind": platform, #回测盘才需要此参数,代表回测所用历史数据所属交易所.实盘应该注释此参数
            "symbols": symbols,
            "account": account,
            ...
            "direct_kline_update": True,     #回测盘这里必须是true
            "direct_orderbook_update": True, #回测盘这里必须是true
            "direct_trade_update": True,     #回测盘这里必须是true
            "direct_ticker_update": True     #回测盘这里必须是true
        }
        self.trader = self.create_gateway(**params)
        
        #=====================================================
        #创建第二个交易接口
        platform = config.accounts[1]["platform"] #交易所
        account = config.accounts[1]["account"] #此参数回测盘不用,不过为了统一都写上不影响
        access_key = config.accounts[1]["access_key"] #此参数回测盘不用,不过为了统一都写上不影响
        secret_key = config.accounts[1]["secret_key"] #此参数回测盘不用,不过为了统一都写上不影响
        symbols = config.markets[platform]["symbols"]
        # 交易模块参数
        params = {
            "strategy": config.strategy,
        ...
        self.trader1 = self.create_gateway(**params)
