...
class HistoryDataFeed:
    ...
    async def start(cls):
        ...
        run_loop_thread = Thread(target=cls.new_loop_thread, args=(thread_loop,)) #新起线程运行事件循环, 防止阻塞主线程
        ...
        while et <= end_time: #每次从数据库中读取一段时间的数据
            ...
            @async_method_locker("HistoryDataFeed.start.task") #上锁
            ...
            #在新线程中运行
            #coro = task(pd_list)
            #future = asyncio.run_coroutine_threadsafe(coro, thread_loop)
            #r = future.result() #注释掉,因为无须等待